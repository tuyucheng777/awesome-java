...
* [ColdBox](https://github.com/coldbox/coldbox-platform)：ColdBox是一个基于约定的HMVC Web开发框架，适用于BoxLang JVM语言和ColdFusion。
...
* [LeapFramework](https://github.com/leapframework/framework)：Leap是一个开源的Java全栈开发框架，目标是简化企业应用开发。
...
* [Light4j REST](https://github.com/networknt/light-rest-4j)：Light4j REST是一个基于Light4j构建的RESTful框架，同时支持Swagger 2.0和OpenAPI 3.0。
...
* [Summer](https://github.com/yale8848/Summer)：Summer是一个连接JAX-RS和Vertx的Web服务器。
...
* [Fluxzero](https://fluxzero.io/)：Fluxzero是一个云运行时，用于构建后端，而无需基础设施开销。
...
* [DGate](https://github.com/DTeam-Top/dgate)：DGate是基于Vertx的API Gateway。
* [Membrane](https://github.com/membrane/api-gateway)：Membrane是用Java编写的REST、OpenAPI、GraphQL和SOAP的API网关，由Predic8公司开源。
* [OmniGateway](https://gitee.com/vakinge/omniGateway)：OmniGateway是一个全能应用网关。
...
* [Phi Accrual Failure Detector](https://github.com/komamitsu/phi-accrual-failure-detector)：Akka的Phi累积失败检测器的移植版本。
...
* [Spring Cloud Circuit Breaker](https://github.com/spring-cloud/spring-cloud-circuitbreaker)：Spring Cloud熔断器API及实现。
...
* [FlowControl](https://gitee.com/shiqiyue/flow-control)：Spring Boot流量控制，控制用户访问API的次数。
* [Token Bucket](https://github.com/bbeck/token-bucket)：该库提供了令牌桶算法的实现。
* [IP Limiter](https://gitee.com/laofeng/ip-limiter)：IP限流平台，自动对异常访问的IP进行限制。
...
* [Token Bucket](https://github.com/nurkiewicz/token-bucket)：Java EE令牌桶算法实现。
* [Rate Limiter](https://github.com/Ace0777/rate-limiter)：Rate Limiter是一个具有流量控制功能的生产级API网关。
* [Limiter](https://github.com/iMouseWu/limiter)：主要用于开放平台的接口限流框架。
* [Java Rate Limiter](https://github.com/cpthack/java-rate-limiter)：基于Guava的RateLimiter、Redis封装了并发控制、限流管理功能。
...
* [Parquet4s](https://github.com/mjakubowski84/parquet4s)：Parquet4s是一个简单的Parquet I/O库，允许你在Scala中轻松地读取和写入Parquet文件。
...
* [Stream Ops](https://github.com/nanosai/stream-ops-java)：Stream Ops是一个完全可嵌入的Java数据流引擎和流处理API。
...
* [Apache Daffodil](https://github.com/apache/daffodil)：Daffodil是DFDL规范的开源实现。
...
* [AtlasMap](https://github.com/atlasmap/atlasmap)：AtlasMap是一种具有交互式网页用户界面的数据映射解决方案，简化了Java、XML和JSON数据源之间的集成配置。
...
* [Jumbune](https://github.com/Impetus/jumbune)：Jumbune是一个开源的大数据APM，可帮助提高云端和本地数据中心的数据操作性能。
...
* [Flink Remote Shuffle](https://github.com/flink-extended/flink-remote-shuffle)：该项目实现了用于Flink批量数据处理的远程洗牌服务。
...
* [Redis Simple Message Queue](https://github.com/igr/jrsmq)：一个轻量级的Java消息队列，无需专用队列服务器，只需一个Redis服务器即可。
* [Mongo Queue Java](https://github.com/yonderblue/mongo-queue-java)：使用MongoDB作为后端的Java消息队列。
* [HazelcastMQ](https://github.com/mpilone/hazelcastmq)：HazelcastMQ在内存数据网格Hazelcast提供的基本队列和主题数据结构之上，提供了一个简单的消息传递层。
* [Kompics](https://github.com/kompics/kompics)：Kompics是一种用于构建分布式系统的消息传递组件模型。
* [ArkTwin](https://github.com/arktwin/arktwin)：ArkTwin是一个分布式消息传递框架。
* [Realtime Message System](https://github.com/goodrain/realtime-message-system)：基于Akka的分布式实时消息交换系统。
...
* [FrankenPaxos](https://github.com/mwhittaker/frankenpaxos)：Paxos协议的各种状态机复制实现。
...
* [Gossip](https://github.com/geertvos/gossip)：基于Gossip网络的集群框架的Java实现。
* [Gossip](https://github.com/dongjinyong/Gossip)：Java实现的Gossip协议，基于Cassandra V1.1.1源码修改实现。
...
* [MongoT](https://github.com/mongodb/mongot)：MongoT为MongoDB提供全文和向量搜索功能。
* [JSearch](https://github.com/ysc/jsearch)：JSearch是一个高性能的全文检索工具包，基于倒排索引。
...
* [Apache S2Graph](https://github.com/apache/incubator-s2graph)：S2Graph是一个基于HBase构建的分布式、可扩展的OLTP图数据库，支持超大图的快速遍历，由Kakao开源。
...
* [Apache Blur](https://github.com/apache/incubator-retired-blur)：Blur是一个基于Lucene、Hadoop和Thrift构建的NoSQL数据存储系统。
...
* [FusionDB](https://www.fusiondb.com/)：FusionDB是一个简单且强大的多模态数据库。
...
* [JavaChain](https://github.com/shenyubao/javachain)：JavaChain用于快速搭建LLM应用。
...
* [Java ChatGPT API](https://github.com/linweiyuan/java-chatgpt-api)：ChatGPT Java非官方API。
...
* [Anthropic Kotlin SDK](https://github.com/xemantic/anthropic-sdk-kotlin)：Anthropic SDK的非官方Kotlin多平台版本。
...
* [AI DIAL](https://github.com/epam/ai-dial-core)：AI DIAL为不同的聊天补全和嵌入模型、助手和应用程序提供统一的API，由EPAM开源。
...
* [Dify Java Client](https://github.com/yuanbaobaoo/dify-java-client)：简单易用的Dify Java客户端。
...
* [MateClaw](https://github.com/matevip/mateclaw)：MateClaw是一款基于Java、Vue 3构建的个人AI操作系统，由Spring AI Alibaba提供技术支持。
...
* [Copilot SDK Java](https://github.com/github/copilot-sdk)：GitHub Copilot CLI的Java SDK。
...
* [Claude Code Java](https://github.com/vincentchen2026/claude-code-java)：Claude Code的Java版本实现。
...
* [Conversation Kit](https://github.com/pdtyreus/conversation-kit)：Conversation Kit旨在为处理人类用户与聊天机器人和语音代理之间的对话提供灵活的结构。
...
* [jLibSVM](https://github.com/davidsoergel/jlibsvm)：Java中支持向量机的高效训练。
...
* [JDMP](https://github.com/jdmp/java-data-mining-package)：JDMP是一个用于数据分析和机器学习的开源Java库。
* [MLTK](https://github.com/yinlou/mltk)：MLTK是一系列监督式机器学习算法的集合，旨在直接用于模型训练和后续开发。
...
* [Fedlearn](https://github.com/fedlearnAI/fedlearn)：联邦学习系统，包括常用算法和通用训练推理系统框架。
...
* [MGO](https://github.com/openmole/mgo)：MGO是一个纯函数式Scala库，用于进化/遗传算法。
...
* [TraClusAlgorithm](https://github.com/luborliu/TraClusAlgorithm)：Java实现的TraClus算法。
...
* [JCRFsuite](https://github.com/vinhkhuc/jcrfsuite)：CRFsuite的Java接口。
* [CRF4j](https://github.com/zhifac/crf4j)：Java的CRF模型训练和测试。
* [CRF](https://github.com/witgo/CRF)：CRF是条件随机场的Java实现，它是一种用于从带标签的示例序列中学习的算法。
...
* [JPMML SkLearn](https://github.com/jpmml/jpmml-sklearn)：用于将Scikit-Learn管道转换为PMML的Java库和命令行应用程序。
* [JPMML LightGBM](https://github.com/jpmml/jpmml-lightgbm)：用于将LightGBM模型转换为PMML的Java库和命令行应用程序。
* [JPMML SparkML](https://github.com/jpmml/jpmml-sparkml)：用于将Apache Spark ML管道转换为PMML的Java库和命令行应用程序。
* [JPMML Model](https://github.com/jpmml/jpmml-model)：用于PMML的Java类模型API。
* [JPMML XGBoost](https://github.com/jpmml/jpmml-xgboost)：用于将XGBoost模型转换为PMML的Java库和命令行应用程序。
* [JPMML Evaluator Spark](https://github.com/jpmml/jpmml-evaluator-spark)：用于Apache Spark的PMML评估器库。
* [JPMML TensorFlow](https://github.com/jpmml/jpmml-tensorflow)：用于将TensorFlow模型转换为PMML的Java库和命令行应用程序。
...
* [Puck](https://github.com/dlwh/puck)：Puck是一款高速、高精度的自然语言解析器。
* [BIDParse](https://github.com/BIDData/BIDParse)：GPU加速的自然语言解析器，由伯克利BID实验室研发。
...
* [THULAC4j](https://github.com/yizhiru/thulac4j)：THULAC4j是THULAC的高效Java 8实现。
...
* [Kotori](https://github.com/wanasit/kotori)：Kotori是一个用Kotlin编写的日语分词器和词法分析引擎。
...
* [Py4j](https://github.com/TFdream/py4j)：一个用于将中文转换为拼音的开源Java库。
* [Java Pinyin](https://github.com/frapples/java-pinyin)：一个汉字与拼音互转的工具库，提供简单的分词功能。
* [UniPinyin](https://github.com/nillith/UniPinyin)：Java汉语拼音查询工具。
...
* [OpenIE](https://github.com/dair-iitd/OpenIE-standalone)：OpenIE是华盛顿大学和印度理工学院德里分校联合开发的开放信息抽取系统。
...
* [Text Classification SVM](https://github.com/hankcs/text-classification-svm)：HanLP基于SVM的文本分类模块。
...
* [jFuzzyLogic](https://github.com/pcingola/jFuzzyLogic)：Java中最完整的模糊逻辑库。
...
* [Google Speech To Text](https://github.com/opensourceteams/google-sdk-speech-to-text)：Google语音转文字SDK。
